...
public abstract class BatchProcessor<INPUT, Target> extends DefaultAmbitProcessor<INPUT, IBatchStatistics> implements
	IBatchProcessor<INPUT, Target, IBatchStatistics> {
    ...
    public void setProcessorChain(ProcessorsChain<Target, IBatchStatistics, IProcessor> processor) {
	...
	try {
		...
		try {
		    ...
		    stats.incrementTimeElapsed(IBatchStatistics.RECORDS_STATS.RECORDS_READ, System.currentTimeMillis()
			    - ms);
		} catch (Exception x) {
		    stats.increment(IBatchStatistics.RECORDS_STATS.RECORDS_ERROR);
		    stats.incrementTimeElapsed(IBatchStatistics.RECORDS_STATS.RECORDS_ERROR, System.currentTimeMillis()
			    - ms);
		    continue;
		}
		...
		try {
		    ...
		    stats.incrementTimeElapsed(IBatchStatistics.RECORDS_STATS.RECORDS_PROCESSED,
