...
public class AmbitLogger {
    ...
    /**
     * Constructs a LoggingTool which produces logger lines indicating them to
     * be for the Class of the <code>Object</code>.
     * 
     * @param object
     *            Object from which the logger messages originate
     */
	...
	try { // NOPMD
	    org.apache.log4j.PropertyConfigurator.configure(AmbitLogger.class.getResource(file));
	...
	try {
	    /*
	     * Next line is required to not have the compiler trip over the the
	     * catch clause later, which in turn is needed on runtime when
	     * security is checked, which is with the PluginManager, as it uses
	     * the java.net.URLClassLoader.
	     */
	    // if (false) throw new ClassNotFoundException(); // NOPMD

	    log4jLogger = (org.apache.log4j.Category) org.apache.log4j.Category.getInstance(classname);
	    /*
	     * } catch (ClassNotFoundException e) { tostdout = true;
     ...
     * library, but is available for convenience.
     */

    /**
     * Outputs system properties for the operating system and the java version.
     * More specifically: os.name, os.version, os.arch, java.version and
     * java.vendor.
     */
     ...
     * calling <code>debug(Throwable)</code>. The default value is
     * DEFAULT_STACK_LENGTH.
     * 
     * @param length
     *            the new stack length
     * 
     ...
     * Throwable it will output the trace. Otherwise it will use the toString()
     * method.
     * 
     * @param object
